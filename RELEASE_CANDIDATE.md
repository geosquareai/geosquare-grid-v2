...
`src/db/registry.db` generated by `scripts/generate_release_candidates.py` is intentionally unsigned and not loadable as-is. Review all generated profiles, boundary hashes, CRS definitions, and scale metadata (e.g. via `sqlite3 src/db/registry.db .dump`); then sign it locally:
...
Then review the resulting diff and sign the new database. The candidate generator pins exact artifact bytes and the local PROJ database/version; an upgrade to PROJ requires a fresh candidate generation and signature.
