...
| Extra | Provides |
|---|---|
| `registry` | Ed25519 signature verification and PROJ resource checks |
...
| `dev` | Pytest and Hypothesis test runners |

## Registry at a glance (new: SQLite-backed)
...
`DbRegistryLoader` (`geosquare_v2.db`) is fail-closed: it verifies the database's signature against a trusted key, then checks the PROJ environment, profile CRS definitions, and (by default) boundary hashes before it returns any profile. `DomainRegistry` and `ReleaseProfile` — the in-memory objects your code actually consumes — are unchanged from before.

Boundary GeoJSON files are provenance metadata only; no grid calculation (indexing, encoding, geometry, or polyfill) reads them. If you want to ship just `registry.db` + `registry.db.sig` without the `boundaries/*.geojson` files, pass `verify_boundaries=False` and omit `boundary_root`:
...
To quickly see which domains a `registry.db` file contains without doing full signature/CRS/boundary verification, use `list_registry_domains`:
...
This is a metadata-only listing intended for inspection/CLI use (it does not check the `.sig` file, PROJ resources, or boundary hashes). Use `DbRegistryLoader.load()` when you need verified profiles for actual application logic.
...
```zsh
# 1. Regenerate the candidate database from source boundaries + profile parameters
.venv/bin/python scripts/generate_release_candidates.py

# 2. Sign the freshly generated database (never skip this before distributing it)
...
scripts/
  generate_release_candidates.py  Builds an unsigned candidate registry.db from boundaries + profile parameters
  sign_registry_db.py             Signs registry.db, producing registry.db.sig
  migrate_registry_to_db.py       Converts a legacy JSON registry tree into registry.db
tests/                            Focused conformance tests, including registry migration/loader/signing tests
