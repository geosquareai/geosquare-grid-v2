...
```
src/db/registry.db       <- databasenya
src/db/registry.db.sig   <- tanda tangan digital untuk database ini
```
...
| `src/db/registry.db` & `registry.db.sig` | Database registry yang sudah jadi dan sudah ditandatangani, siap dipakai |
...
registry = DbRegistryLoader(
    project_root / "src/db",                                    # folder tempat registry.db & .sig
...
sqlite3 src/db/registry.db ".dump"
...
- Kalau butuh cek isi database secara manual, pakai `sqlite3 src/db/registry.db` lalu jalankan query SQL biasa (misalnya `SELECT * FROM domains;`).
